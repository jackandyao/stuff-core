package com.qbao.aisr.stuff.crawler.cmp.http;
...
import com.qbao.aisr.stuff.crawler.cmp.http.builder.HCB;
...
import com.qbao.aisr.stuff.crawler.cmp.http.common.Utils;
...
public class HttpClientUtil{
	...
	private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
		...
		try {
			...
			if(!StringUtils.isEmpty(config.ip())) {
				String[] ipport = config.ip().split(":");
				if(ipport.length == 2){
                    //设置代理ip和端口
